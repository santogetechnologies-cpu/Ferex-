...
const assignedStudents = [
  {
    ...
    course: 'MSc Data Science (2026 Intake)',
    status: 'CAS Letter Issued',
    risk: 'Low Risk',
    completion: 85,
    paymentStatus: 'Paid ₹1,50,000 (Read Only)',
    documents: ['Passport Scan', 'SOP Draft v2', 'IELTS 7.5 TRF'],
    notes: 'CAS request approved by UKVI team. Bank solvency verified.'
  },
  {
    id: 'STU-889',
    name: 'Rahul Verma',
    ...
    status: 'Visa Biometric Scheduled',
    risk: 'Low Risk',
    completion: 70,
    paymentStatus: 'Paid ₹2,10,000 (Read Only)',
    documents: ['GIC Receipt', 'Academic Transcripts', 'SOP v3'],
    notes: 'Canada Study Permit file lodged via SDS portal.'
  },
  {
    id: 'STU-912',
    name: 'Ananya Roy',
    ...
    status: 'Deposit Paid (€2,000)',
    risk: 'Medium Risk',
    completion: 55,
    paymentStatus: 'Deposit Paid €2,000 (Read Only)',
    documents: ['Degree Cert', 'Bank Statement', 'LOR 1 & 2'],
    notes: 'Transcripts verified. Awaiting final CAS clearance.'
  },
];
          ...
          <Card key={student.id} className="p-6 border border-slate-200/80 shadow-xs hover:shadow-lg transition-all space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                <span className="text-3xl">{student.flag}</span>
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-black text-slate-900">{student.name}</h3>
                    <span className="text-[10px] font-black uppercase text-[#6A1B2E] bg-[#6A1B2E]/10 px-2.5 py-0.5 rounded-full border border-[#6A1B2E]/20">{student.id}</span>
                    <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${student.risk === 'Low Risk' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>{student.risk}</span>
                  </div>
                  <p className="text-xs font-semibold text-slate-500 mt-0.5">{student.uni} • {student.course}</p>
                </div>
              </div>

              <div className="text-right">
                <span className="text-xs font-black text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 block">{student.status}</span>
                <span className="text-[10.5px] font-bold text-slate-400 mt-1 block">Application Completion: {student.completion}%</span>
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-2 rounded-full bg-[#6A1B2E]" style={{ width: `${student.completion}%` }} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs font-semibold text-slate-600">
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                ...
                <span className="font-black text-slate-900 block">{student.paymentStatus}</span>
              </div>

              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1 md:col-span-2">
                <span className="text-[9.5px] font-extrabold uppercase text-slate-400 block">Verified Document Files</span>
            ...
            <div className="flex items-center justify-between border-t border-slate-100 pt-3 text-xs font-bold">
              ...
              <button onClick={() => setSelectedStudent(student)} className="text-[#6A1B2E] font-black hover:underline flex items-center gap-1">
