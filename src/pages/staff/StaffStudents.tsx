...
import { Users, CheckCircle2, Lock, Eye, X, FileText, GraduationCap } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 flex items-center gap-2">
            <Users className="w-5 h-5 text-[#58051E]" /> Assigned Student Profiles
          </h1>
              ...
              <Card key={student.id} className="p-6 border border-slate-200/80 shadow-xs hover:shadow-lg transition-all space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#58051E]/10 border border-[#58051E]/20 text-[#58051E] flex items-center justify-center shrink-0">
                      <GraduationCap className="w-5 h-5" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-base font-black text-slate-900">{stuName}</h3>
                        <span className="text-[10px] font-black uppercase text-[#58051E] bg-[#58051E]/10 px-2.5 py-0.5 rounded-full border border-[#58051E]/20">{stuId.slice(0, 8)}</span>
                ...
                <div className="space-y-1">
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-2 rounded-full bg-[#58051E]" style={{ width: `${completion}%` }} />
                  </div>
                </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-0.5">
                      {['Passport Scan', 'Academic Transcripts', 'SOP Statement'].map((doc, idx) => (
                        <span key={idx} className="text-[10.5px] font-bold bg-white text-slate-800 px-2.5 py-0.5 rounded-lg border border-slate-200 flex items-center gap-1">
                          <FileText className="w-3 h-3 text-[#58051E]" /> {doc}
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between border-t border-slate-100 pt-3 text-xs font-bold">
                  ...
                  <button onClick={() => setSelectedStudent({ name: stuName, id: stuId, country, uni, course, status, notes: 'Assigned counselor review in progress.' })} className="text-[#58051E] font-black hover:underline flex items-center gap-1">
            ...
            <motion.div initial={{ translateX: '100%' }} animate={{ translateX: 0 }} exit={{ translateX: '100%' }} transition={{ duration: 0.25 }} className="fixed top-0 right-0 h-screen w-full max-w-md bg-white z-50 shadow-2xl p-6 overflow-y-auto space-y-4 text-left">
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100">
                ...
                <textarea
                  ...
                  className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-[#58051E]/20"
                />
                <Button
                  size="sm"
                  className="w-full bg-[#58051E] hover:bg-[#430316] text-xs font-bold"
