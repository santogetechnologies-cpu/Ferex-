  ...
  return (
    <div className="space-y-6 text-left antialiased select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 flex items-center gap-2">
            <Bell className="w-5 h-5 text-[#58051E]" /> Enterprise Notification Center
          </h1>
        ...
        <Button size="sm" variant="outline" className="text-xs font-bold cursor-pointer" onClick={markAllRead}>
      ...
      <div className="flex gap-2 border-b border-slate-200/80 pb-2">
        {categories.map(cat => (
          <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${activeCategory === cat ? 'bg-[#58051E] text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{cat}</button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {filteredNotifications.map(n => {
          ...
          return (
            <Card key={n.id} className={`p-4 border transition-all space-y-2 ${n.unread ? 'border-[#58051E]/40 bg-[#58051E]/5 shadow-xs' : 'border-slate-200 bg-white'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-xl bg-[#58051E]/10 text-[#58051E] flex items-center justify-center font-bold">
