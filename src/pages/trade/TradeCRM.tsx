...
import { Building2, Search, Plus, Eye, Edit3, Trash2, X, CheckCircle2, Mail, Phone, KeyRound, Copy, ShieldAlert, Ship, FileCheck, DollarSign, Globe } from 'lucide-react';
    ...
    <div className="space-y-6 text-left antialiased">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <Building2 className="w-5 h-5 text-[#58051E]" /> Global Trade CRM Directory
          </h1>
      ...
      <Card className="p-4 border border-slate-200/70 shadow-xs flex flex-col sm:flex-row gap-3 items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search company, person, or email..." className="w-full h-9 pl-9 pr-4 bg-slate-100/70 border border-slate-200 rounded-xl text-xs font-semibold text-slate-900 focus:bg-white focus:outline-none focus:border-[#58051E]" />
        </div>
            ...
            <motion.div initial={{ translateX: '100%' }} animate={{ translateX: 0 }} exit={{ translateX: '100%' }} transition={{ duration: 0.25 }} className="fixed top-0 right-0 h-screen w-full max-w-lg bg-white z-50 shadow-2xl p-6 overflow-y-auto">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
                <div>
                  ...
                  <span className="text-[10px] font-bold text-[#58051E] uppercase">Global Trade Partner Dossier</span>
                </div>
              ...
              <div className="space-y-5 text-left text-xs">
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                    <Ship className="w-3.5 h-3.5 text-[#58051E]" /> Associated Maritime Shipments ({partnerShipments.length})
                  </h4>
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                    <FileCheck className="w-3.5 h-3.5 text-[#58051E]" /> Letters of Credit ({partnerLCs.length})
                  </h4>
                  {partnerLCs.length === 0 ? (
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                    <DollarSign className="w-3.5 h-3.5 text-[#58051E]" /> Commercial Invoices ({partnerInvoices.length})
                  </h4>
                ...
                <Button size="sm" className="w-full text-xs font-bold bg-[#58051E] hover:bg-[#430316]" onClick={() => setSelectedCompany(null)}>
                  Close Profile
                </Button>
              </div>
            </motion.div>
