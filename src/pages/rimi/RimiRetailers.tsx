...
import { Store, Search, Plus, Eye, Trash2, X, CheckCircle2, KeyRound, Copy, ShieldAlert, ShoppingBag, DollarSign } from 'lucide-react';
...
export const RimiRetailers: React.FC = () => {
  ...
  const handleAddRet = async (e: React.FormEvent) => {
    ...
    showToastMsg(`Added supermarket retailer ${newRet.store}`);
    setNewRet({ store: '', owner: '', email: '', phone: '', city: 'Mumbai Suburban', creditLimit: 500000 });
    await loadData();
  ...
  const handleProvisionCredentials = async (ret: any) => {
    const existing = getRimiCustomerCredentials(ret.rawId || ret.id);
    if (existing) {
    ...
    const cred = await provisionRimiCustomerLogin({
      ...
      email: ret.email,
      name: ret.store,
      business_name: ret.store,
      contact_person: ret.owner
    });
    setActiveCredential(cred);
    showToastMsg(`Provisioned retailer portal login for ${ret.store}`);
    loadData();
  };

  const handleOpenDossier = async (ret: any) => {
    setSelectedRet(ret);
    try {
      ...
      setRetOrders(orders.filter((o: any) => o.distributor_name?.includes(ret.store) || o.distributor?.business_name?.includes(ret.store)));
      setRetPayments(payments.filter((p: any) => p.distributor_name?.includes(ret.store) || p.distributor?.business_name?.includes(ret.store)));
    } catch {
      setRetOrders([]);
      setRetPayments([]);
    }
    ...
    <div className="space-y-6 text-left antialiased">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <Store className="w-5 h-5 text-[#58051E]" /> Supermarket Retail Chains & Store Accounts
          </h1>
          <p className="text-xs font-semibold text-slate-500 mt-1">
            Rimi Cold Chain Console • Modern retail outlets, store procurement leads, and credit terms.
          </p>
        </div>
      ...
      <Card className="p-4 border border-slate-200/70 shadow-xs flex items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search store name, owner, or city..." className="w-full h-9 pl-9 pr-4 bg-slate-100/70 border border-slate-200 rounded-xl text-xs font-semibold text-slate-900 focus:bg-white focus:outline-none focus:border-[#58051E]" />
        </div>
        <span className="text-xs font-bold text-slate-400">{filteredRet.length} Verified Retailers</span>
      </Card>

      {loading ? (
        <div className="p-8 text-center text-xs font-bold text-slate-400">Loading retail partners...</div>
      ) : filteredRet.length === 0 ? (
        <Card className="p-12 text-center border border-dashed border-slate-200">
          <Store className="w-10 h-10 text-slate-300 mx-auto mb-3" />
          <h3 className="text-sm font-black text-slate-800">No retailer accounts found</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">There are no supermarket stores registered in this view. Add one below.</p>
            ...
            <Card key={r.id} className="p-5 border border-slate-200/70 shadow-xs space-y-4 hover:border-slate-300 transition-all flex flex-col justify-between">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-[#58051E] mt-0.5">{r.city}</p>
                </div>
                <div className="space-y-1 text-xs text-slate-500 pt-1">
                  <div>Owner / Lead: <strong className="text-slate-800">{r.owner}</strong></div>
                  <div>Email: <strong className="text-slate-800">{r.email}</strong></div>
                  <div>Phone: <strong className="text-slate-800">{r.phone}</strong></div>
                </div>
              </div>

              <div className="space-y-2.5 pt-3 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black text-slate-900">{r.creditLimit}</span>
                  <button onClick={() => handleDeleteRet(r.rawId)} className="p-1.5 text-slate-400 hover:text-red-600 rounded" title="Delete Store">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => handleProvisionCredentials(r)}
                  >
                    <KeyRound className="w-3 h-3 mr-1 text-amber-300" />
                    {r.hasCredentials ? 'View Login' : 'Provision Login'}
                  </Button>
                </div>
              </div>
            </Card>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-2xl z-50 border border-slate-100 p-6">
              ...
              <form onSubmit={handleAddRet} className="space-y-3">
                <div>
                  ...
                  <input type="text" required value={newRet.store} onChange={(e) => setNewRet({ ...newRet, store: e.target.value })} placeholder="e.g. Modern Bazaar Bandra West" className="w-full h-9 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold" />
                </div>
                <div>
                  <label className="block text-[10px] font-extrabold text-slate-400 uppercase mb-1">Store Lead / Contact</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-extrabold text-slate-400 uppercase mb-1">City / Suburban Area</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-extrabold text-slate-400 uppercase mb-1">Credit Facility (₹)</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-extrabold text-slate-400 uppercase mb-1">Store Email</label>
                    <input type="email" required value={newRet.email} onChange={(e) => setNewRet({ ...newRet, email: e.target.value })} className="w-full h-9 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-extrabold text-slate-400 uppercase mb-1">Phone</label>
                    <input type="text" value={newRet.phone} onChange={(e) => setNewRet({ ...newRet, phone: e.target.value })} className="w-full h-9 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold" />
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-2xl z-50 border border-slate-100 p-6 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-slate-900">Retailer Portal Login Access</h3>
              ...
              <div className="p-3 bg-amber-50 border border-amber-200/80 rounded-xl text-xs font-semibold text-amber-800 flex items-start gap-2.5">
                ...
                <div>
                  ...
                  Store manager will set a permanent password upon initial sign in.
                </div>
              </div>
          ...
          <>
            ...
            <motion.div initial={{ translateX: '100%' }} animate={{ translateX: 0 }} exit={{ translateX: '100%' }} transition={{ duration: 0.25 }} className="fixed top-0 right-0 h-screen w-full max-w-lg bg-white z-50 shadow-2xl p-6 overflow-y-auto">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
                <div>
                  <h3 className="text-sm font-black text-slate-900">{selectedRet.store}</h3>
                  <span className="text-[10px] font-bold text-[#58051E] uppercase">Retail Outlet Dossier</span>
                </div>
              ...
              <div className="space-y-5 text-left text-xs">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[10px] font-black text-[#58051E] uppercase">{selectedRet.id}</span>
                ...
                <div className="p-4 bg-slate-50 rounded-xl space-y-2 text-xs font-semibold text-slate-700">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400">Email:</span>
                    <span className="font-bold text-slate-900">{selectedRet.email}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Phone:</span>
                    <span className="font-bold text-slate-900">{selectedRet.phone}</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                    <ShoppingBag className="w-3.5 h-3.5 text-[#58051E]" /> Store Orders ({retOrders.length})
                  </h4>
                  {retOrders.length === 0 ? (
                      ...
                      {retOrders.map((o: any) => (
                        <div key={o.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200/60 space-y-1">
                          ...
                          <div className="flex justify-between text-[11px] text-slate-500">
                            <span>Delivery: {o.delivery_date || 'Reefer Route'}</span>
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                    <DollarSign className="w-3.5 h-3.5 text-[#58051E]" /> Payment Receipts ({retPayments.length})
                  </h4>
                  {retPayments.length === 0 ? (
                    <div className="p-3 bg-slate-50 rounded-xl text-slate-400 text-center font-medium">No recorded settlements</div>
                  ) : (
                    <div className="space-y-2">
                      {retPayments.map((p: any) => (
                ...
                <Button size="sm" className="w-full text-xs font-bold bg-[#58051E] hover:bg-[#430316]" onClick={() => setSelectedRet(null)}>
                  Close Dossier
                </Button>
              </div>
            </motion.div>
