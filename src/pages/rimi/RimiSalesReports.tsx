    ...
    <div className="space-y-6 text-left antialiased">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-[#58051E]" /> FMCG Sales & Distribution Audit Reports
          </h1>
        ...
        <Button size="sm" className="bg-[#58051E] hover:bg-[#430316] text-xs font-bold" onClick={() => showToastMsg('Generating Master FMCG Sales Audit PDF...')}>
          ...
          return (
            <Card key={idx} className="p-4 border border-slate-200/80 shadow-xs flex flex-col justify-between">
              <div className="flex items-center justify-between mb-1">
                ...
                <IconC className="w-4 h-4 text-[#58051E]" />
              </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                activeFilter === tab.key ? 'bg-[#58051E] text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
              {tab.label}
            </button>
          ...
          <Card key={idx} className="p-5 border border-slate-200/70 shadow-xs flex flex-col justify-between space-y-4 hover:border-slate-300 transition-all">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-[#58051E] uppercase">{r.code}</span>
              ...
              <div className="p-3 bg-slate-50 rounded-xl space-y-1 text-xs">
                ...
                <div className="flex justify-between font-bold text-slate-700">
                  ...
                  <span className="text-[#58051E] font-black">{r.totalValue}</span>
                </div>
              </div>
            </div>
            <div className="flex gap-2 pt-2 border-t border-slate-100">
              ...
              <Button size="sm" className="flex-1 text-xs font-bold bg-[#58051E] hover:bg-[#430316]" onClick={() => showToastMsg(`Downloading ${r.code}.pdf`)}>
      ...
      <Card className="p-6 border border-slate-200/70 shadow-xs space-y-4">
        <h3 className="text-sm font-black text-slate-900 flex items-center gap-2">
          <Package className="w-4 h-4 text-[#58051E]" /> Live SKU Category Volume Matrix
        </h3>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl bg-white rounded-2xl shadow-2xl z-50 border border-slate-100 p-6 space-y-4 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div>
                  <span className="text-[10px] font-black uppercase text-[#58051E]">{selectedReport.code}</span>
              ...
              <div className="pt-3 border-t border-slate-100 flex gap-2">
                ...
                <Button size="sm" className="flex-1 text-xs font-bold bg-[#58051E] hover:bg-[#430316]" onClick={() => { showToastMsg(`Exported ${selectedReport.code}.pdf`); setSelectedReport(null); }}>
