    ...
    <div className="space-y-6 text-left antialiased select-none max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-3">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase bg-[#58051E]/10 text-[#58051E] px-2.5 py-0.5 rounded-full border border-[#58051E]/20">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-bold">
            {(['Production', 'Staging', 'Development'] as const).map(item => (
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${env === item ? 'bg-[#58051E] text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
              >
                {item}
              </button>
            ))}
          </div>
          <Button size="sm" className="bg-[#58051E] hover:bg-[#430316] text-xs font-bold shadow-md shadow-rose-950/10" onClick={() => setShowTerminal(true)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Lighthouse SEO Score', value: '100 / 100', sub: 'Structured JSON-LD Schema', color: 'text-[#58051E]' },
        ].map((card, idx) => (
        ...
        <div className="bg-slate-800 border-b border-slate-700 px-4 py-3 flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-700">
              <button onClick={() => setViewport('desktop')} className={`p-1.5 rounded-lg transition-all cursor-pointer ${viewport === 'desktop' ? 'bg-[#58051E] text-white' : 'text-slate-400 hover:text-white'}`}><Laptop className="w-4 h-4" /></button>
              <button onClick={() => setViewport('tablet')} className={`p-1.5 rounded-lg transition-all cursor-pointer ${viewport === 'tablet' ? 'bg-[#58051E] text-white' : 'text-slate-400 hover:text-white'}`}><Tablet className="w-4 h-4" /></button>
              <button onClick={() => setViewport('mobile')} className={`p-1.5 rounded-lg transition-all cursor-pointer ${viewport === 'mobile' ? 'bg-[#58051E] text-white' : 'text-slate-400 hover:text-white'}`}><Smartphone className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
          ...
          <div className={`bg-white rounded-2xl shadow-2xl transition-all duration-300 overflow-hidden border border-slate-200 ${viewport === 'desktop' ? 'w-full max-w-5xl' : viewport === 'tablet' ? 'w-[640px]' : 'w-[360px]'}`}>
            <div className="p-6 space-y-6 text-left">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-xl bg-[#58051E] text-white flex items-center justify-center font-black text-xs">FX</div>
                  <span className="font-black text-slate-900 text-sm">{activeProject.title}</span>
                </div>
                <div className="flex items-center gap-3 text-xs font-bold text-slate-600">
                  <span className="text-[#58051E]">Services</span>
                  <span>Solutions</span>
                  <span>Architecture</span>
                  <span className="bg-[#58051E] text-white px-3 py-1 rounded-lg text-[11px]">{activeProject.progress || 88}% Built</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5 border border-slate-200/80 shadow-xs space-y-3 rounded-2xl bg-white">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
            <Server className="w-4 h-4 text-[#58051E]" /> REST & GraphQL API Health Monitor
          </h3>
        ...
        <Card className="p-5 border border-slate-200/80 shadow-xs space-y-3 rounded-2xl bg-white">
          <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
            <GitBranch className="w-4 h-4 text-[#58051E]" /> Active Git Commits Stream
          </h3>
          <div className="space-y-2 text-xs font-mono">
            {commitsList.map((c, i) => (
