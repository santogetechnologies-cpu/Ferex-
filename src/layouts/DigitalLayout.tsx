          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed top-24 left-1/2 -translate-x-1/2 w-full max-w-xl bg-white rounded-2xl shadow-2xl z-50 border border-slate-100 p-4">
              ...
              <div className="mt-3 space-y-1 max-h-64 overflow-y-auto">
                <span className="text-[10px] font-black uppercase text-slate-400 px-2 block">Quick Agency Shortcuts</span>
                {[
                  { label: 'Clients Directory & B2B Accounts', path: '/digital/clients' },
                  { label: 'Active Projects (Web Dev & Mobile Apps)', path: '/digital/projects' },
                  { label: 'Client Invoices & Billings (₹)', path: '/digital/invoices' },
                  { label: 'Agency Services & Retainers', path: '/digital/services/web-development' },
                ].map((s, idx) => (
                  <button
                    key={idx}
